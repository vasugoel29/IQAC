...
const LinkItem = memo(({ to, children }) => (
  ...
  >
    <Link
      to={to}
      className="block p-2 sm:p-3 rounded-xl hover:bg-gradient-to-r
        from-amber-100/50 to-amber-200/50
        text-amber-900 hover:text-amber-950 transition-all duration-300 
         border border-transparent 
        hover:border-amber-500/30 shadow-lg hover:shadow-amber-500/20 
...
const SectionTitle = memo(({ icon: Icon, children, isExpanded, onToggle }) => (
  <motion.button
    ...
    whileTap={{ scale: 0.98 }}
    onClick={onToggle}
    className="w-full py-2 sm:py-3 px-3 sm:px-4 rounded-xl font-bold text-base sm:text-lg mb-2 sm:mb-3 
      flex items-center justify-between bg-gradient-to-r from-amber-600/40 to-amber-700/40 
      hover:from-amber-500/50 hover:to-amber-600/50 text-amber-950 cursor-pointer transition-all 
      duration-300 border border-amber-500/30 shadow-lg hover:shadow-amber-500/30"
  >
    <span className="flex items-center space-x-2 sm:space-x-3">
      <Icon className="text-amber-700" size={16} />
      <span className="amber-gradient-text">{children}</span>
    </span>
    <motion.span
      ...
      <AnimatePresence initial={false}>
        {isExpanded && (
          <motion.ul
            ...
            className="space-y-2 border-l-2 border-amber-500/30 pl-3 sm:pl-4 ml-2"
          >
            {children}
          </motion.ul>
        )}
    ...
    <>
      ...
      <AnimatePresence>
        {(isSidebarOpen || isDesktop) && (
          <motion.div
            className="fixed inset-0 bg-amber-950/40 z-40 lg:hidden backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
      ...
      <motion.aside
        ...
        className={`
          ...
          text-amber-950 p-4 sm:p-6 z-50
          overflow-y-auto scrollbar-thin
          transform transition-transform duration-300
          border-r border-amber-300/30 shadow-xl
          left-scrollbar  /* Add this class */
...
// Add this style tag to your HTML or inject it using styled-components
